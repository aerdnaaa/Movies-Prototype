...
class CreatePromotion(FlaskForm):
    promotion_title = StringField(label='Promotion Title', validators=[DataRequired(), validate_promotion_title, Length(min=5, max=30)])
    ...
    promotion_valid_start_date = StringField(label='Promotion Start Date', validators=[DataRequired(), validate_start_date])
    promotion_valid_end_date = StringField(label='Promotion End Date', validators=[DataRequired(), validate_end_date])    
...
class ModifyPromotion(FlaskForm):
    promotion_title = StringField(label='Promotion Title', validators=[DataRequired(), Length(min=5, max=30), validate_promotion_title])
    promotion_description = TextAreaField(label='Promotion Description', validators=[DataRequired()])
    promotion_terms_and_condition = TextAreaField(label='Promotion Terms and Condition', validators=[DataRequired()])
    promotion_valid_start_date = StringField(label='Promotion Start Date', validators=[DataRequired(), validate_start_date])
    promotion_valid_end_date = StringField(label='Promotion End Date', validators=[DataRequired(), validate_end_date])    
